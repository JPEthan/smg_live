...
直播和回看地址中的 `token` 具有有效期。旧版本在令牌失效后会让播放器重复请求同一个 `.m3u8` 地址，媒体服务器随即连续返回 `401/403`，只能通过刷新页面获取新地址。

本版本会：

- 读取 JWT `token` 中的 `exp`，在到期前约 90 秒请求新的频道播放地址。
- 发现 `kksmg.com` 的 `.m3u8` 清单返回 `401/403` 时，清除旧缓存并立即更新一次播放令牌。
- 对同一频道的更新请求进行合并并设置冷却时间，避免失败时形成请求循环。
- 重新初始化播放器时强制使用新地址；回看节目会尽量恢复更新前的播放位置和播放状态。
- 在线更新超过 12 秒、服务端仍返回同一个失效地址，或更新后仍持续出现媒体 403 时，直播页面会自动刷新一次。
- 自动刷新使用会话记录和 2 分钟冷却，避免服务持续异常时形成刷新循环；回看节目不会自动刷新，以免丢失所选节目。

控制台成功更新时会显示：

```text
[SMGTV] 已更新播放令牌：令牌即将到期
```

或：

```text
[SMGTV] 已更新播放令牌：媒体服务器返回 403
```
